...
class BatchEpisode(object):
    ...
    def observations(self):
        if self._observations is None:
            ...
            self._observations = torch.from_numpy(observations).to(device)
    ...
    def actions(self):
        if self._actions is None:
            ...
            self._actions = torch.from_numpy(actions).to(device)
    ...
    def rewards(self):
        if self._rewards is None:
            ...
            self._rewards = torch.from_numpy(rewards).to(device)
    ...
    def returns(self):
        if self._returns is None:
            ...
            for i in range(len(self) - 1, -1, -1):
                ...
                returns[i] = return_
            self._returns = torch.from_numpy(returns).to(device)
    ...
    def mask(self):
        if self._mask is None:
            ...
            for i in range(self.batch_size):
                length = len(self._actions_list[i])
                mask[:length, i] = 1.0
            self._mask = torch.from_numpy(mask).to(device)
        return self._mask
