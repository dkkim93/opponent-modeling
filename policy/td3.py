...
class TD3(object):
    ...
    def centralized_task_level_train(self, agent_n, replay_buffer, iterations, batch_size, discount, tau, 
                                     policy_noise, noise_clip, policy_update):
        n_agent = 2
        ...
        for it in range(iterations):
            ...
            x_n, y_n, u_n, r_n, d_n = replay_buffer.centralized_sample(
                batch_size=batch_size, n_agent=n_agent)  # NOTE It returns a list with exp for all agents
            ...
            for i_agent_ in range(n_agent):
                with torch.no_grad():
                    ...
                    next_action = agent_n[i_agent_].policy.actor_target(next_state_n[i_agent_]) + noise
                    next_action = next_action.clamp(-self.max_action, self.max_action)
            ...
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward_n[self.i_agent] + (done_n[self.i_agent] * discount * target_Q).detach()

            # Get current Q estimates
            current_Q1, current_Q2 = self.critic(
                torch.cat(state_n, dim=1), torch.cat(action_n, dim=1))
            ...
            if policy_update:
                ...
                actor_loss = -self.critic.Q1(
                    torch.cat(state_n, dim=1), torch.cat(action_n, dim=1)).mean()
    ...
    def centralized_train(self, agent_n, replay_buffer, iterations, batch_size, discount, tau, 
                          policy_noise, noise_clip, policy_freq):
        n_agent = 4  # NOTE Change for two-box
