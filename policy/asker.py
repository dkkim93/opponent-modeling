...
class Asker(object):
    ...
    def set_actor_input_dim(self):
        ...
        input_dim += 4 * num_random_action
        input_dim += 1 * num_random_action  # Student Q

        return input_dim

    def set_dim(self):
        self.actor_input_dim = self.set_actor_input_dim()
        self.actor_output_dim = 1
    ...
    def select_stochastic_action(self, obs, total_step_count):
        ...
        for i_action in range(self.actor_output_dim):
            self.tb_writer.add_scalar(
                "action/asker_advice" + str(self.i_agent) + "-" + str(i_action),
